...
const CACHE_FILE = path.join(INPUT_DIR, ".image-cache.json");
...
function isUpToDate(inputPath, outPaths, currentHash) {
  try {
    ...
    const outputsExist = [outPaths.png, outPaths.webp, outPaths.avif].every(
      (p) => fs.existsSync(p),
    );
...
(async () => {
  ...
  console.log(
    `images processed: ${summary.processed}, pruned: ${summary.pruned}`,
  );
