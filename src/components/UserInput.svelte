<script>
    ...
    let specialPotionToggles = Object.fromEntries(specialPotions.map((p) => [p.id, false]));
    let environmentBuffToggles = Object.fromEntries(environmentBuffs.map((b) => [b.id, false]));
    let gamepassToggles = Object.fromEntries(gamepasses.map((g) => [g.id, false]));
    let eventToggles = Object.fromEntries(events.map((e) => [e.id, false]));
    let enchantValues = Object.fromEntries(enchants.map((e) => [e.id, 0]));
    ...
    function handleSelect({ option, id }) {
        if (id === "eggs") {
            selectedOptions = { ...selectedOptions, [id]: option.id, rifts: rifts[0]?.id };
    ...
    $: {
        const modifiers = [
            ...
            ...potions.map((potionType) => potionType.potions.find((p) => p.id === selectedOptions[potionType.id])),
            ...milestones.map((milestoneType) => milestoneType.tiers.find((t) => t.id === selectedOptions[milestoneType.id])),
        ...
        stats = calculateStats(modifiers, toggleValues, numericValues);
    }
</script>
